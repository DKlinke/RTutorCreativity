...
### Welcome
<!--The introduction in the thesis differs from the introduction here --> 
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("PrivateLifeComposer1")
```
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("PrivateLifeComposer2")
```
...
[1]: If the average proportion of negative emotions in the total word count increases by one percentage point, we predict that the annual output of outstanding compositions increases by 0.42 pieces in the following year.
[2]: If the average proportion of negative emotions in the total word count increases by 1 percentage , we predict that the annual output of outstanding compositions increases by 0.42 pieces in the following year.
[3]: If the average proportion of negative emotions in the total word count increases by 0.42 percentage points, we predict that the annual output of outstanding compositions increases by 1 piece in the following year.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("InterpretationRegressionCoefficients")
```
...
[1]: Yes. This is a similar interpretation and in general also possible. So it is correct.
[2]: This is a different statement. At this point it is implausible that this interpretation is also correct, therefore further investigation is necessary. 
[3]: Yes. It is not the similiar interpretation, but you can still use it. We can already say, that this is also correct.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("CausalityAndPrediction")
```
...
Correlation therefore does not mean causality. However, correlation is necessary for a causal connection. 
...
Let us just assume for the explanation of the omitted variable bias that we are in a world where model $(2)$ is the long "true" model and model $(1)$ is the short underspecified model. In reality, model $(2)$ is still underspecified and can  **not** be used to describe a causal effect, but we assume this for didactic reasons. An updated model by integrating `tenure` looks like this.

\[ output = \beta_0^{long}+\beta_1^{long}negemo+\beta_2^{long}tenure+u \tag{2}
 \]

We notate model $(1)$ accordingly

\[ output = \beta_0^{short}+\beta_1^{short}negemo+v \tag{1}
 \]
...
\[ v = \beta_2^{long}tenure+u
 \]

One can see well that in the short model `negemo` is correlated with the error $v$, since it includes `tenure`. Thus, in the short model $(1)$, the estimator for $\beta_1^{short}$ is a biased estimator since it violates assumption 2.

On the other hand, since we assume that model 2 is the true model, it also satisfies all the assumptions we made in the previous chapter. Consequently, the estimators for $\beta_1^{long}$ and $\beta_2^{long}$ are unbiased. Given the algebraic connection of the short and long models $\beta_1^{short}=\beta_1^{long}+\beta_2^{long}\delta_1$ (proof can be found in Wooldridge 2020, pp. 114-115), where $\delta_1$ comes from the regression $tenure=\delta_0+\delta_1negemo+\epsilon$ (cf. Wooldridge 2020, pp.84-86), it holds that $\mathbb{E}(\beta_1^{short})=\beta_1^{long}+\beta_2^{long}\delta_1$

and thus the bias of $\beta_1^{short}$ can be derived as follows
 
\[ Bias(\beta_1^{short})=\mathbb{E}(\beta_1^{short})-\beta_1^{long} = \beta_2^{long}\delta_1
 \]
 
Because $\delta_1=corr(negemo,tenure)\cdot\frac{sd(tenure)}{sd(negemo)}$ (proof can be found in Wooldridge 2020, pp.25-26) the direction(sign) of the bias is only depending on the correlation of the two explanatory variables (the sample standard deviations can only take positive values) and $\beta_2^{long}$. The following table based on Wooldridge (2020, p.85) gives a good overview of the bias in $\beta_1^{short}$ when a second relevant explainatory variable is omitted:
...
Quiz: What do you think which sign has $\beta_2^{long}$?

[1]: \(\beta_2^{long}>0\)
[2]: \(\beta_2^{long}<0\) 
[3]: \(\beta_2^{long}=0\) and therefore has no sign.
...
In summary, $\beta_1^{short}$ has a positive bias and systematically overestimates the "true" causal relationship, since both $\beta_2^{long}$ and $corr(tenure,negemo)$ are less than zero. The statement is only valid for the case when we imagine to be in a world where model $(2)$ is the true model. Therefore "true" is also written in quotation marks here.
...
![Figure 1: Causal graph of model 1 (simple world); Source: own diagram.](images/CausalGraphOmitted.png)
...
So, in summary, in Model $(1)$ we measure the direct positive effect of `negemo` on `output` and the indirect positive effect of `negemo` via `tenure` on `output`. The estimator $\hat\beta_1^{short}$ in the short model $(1)$ is thus too large, as it is the sum of these two effects, and does not answer our "research question" correctly.
...
As we will see in the further parts of the problem set, there are also cases where we have to use other tools than the control with variables. This is the case, for example, when we do or cannot observe important variables.
...
One can see that the $\beta_1$ estimate for model $(1)$ with $0.428$ is larger than in model $(2)$ with $0.208$. Thus, $\beta_1^{short}$ indeed systematically overestimates the true effect. Also, the negative effect of `tenure` on `output` already described becomes clear when looking at the estimate for $\beta_2$ with a value of $-3.633$, which moreover is highly significant. The significance of $\beta_1$ decreases in the long model. Here, `negemo` seems to describe `output` less well than `tenure`.

Even though we controlled for `tenure` it is still very unlikely that our estimator $\hat\beta_1$ measures the true causal effect $\beta_1$. So we end our thought experiment in which model $(2)$ is the true model and return to reality.

Other relevant variables for which we do not control are conceivable. In fact, the two previous models have been oversimplified and there exist many more measurable variables for which we can control. The full model with which Borowiecki performed the OLS regression still includes the following control variables: `touring`, `marriage_cohabitation`, `illness`, `letters_annual`, `death_of_relative` and uses fixed effects for `age`, `composer` and `addressee`.


### 2.1.2 Fixed Effects

The models introduced and discussed so far, especially the simple linear regression, were not sufficient to describe the causal effect of `negemo` on `output`. To address the omitted variable bias, we learned about controlling for other relevant variables as a possible solution. For example, by adding `tenure`,`touring`, `marriage_cohabitaion`,`illness`,`letters_annual`,`death_of_relative` to the model. However, this variant might have its limitations especially when one wants to control for unobserved characteristics. Therefore, another option that we will learn about in this subsection is fixed effects regression, which attempts to account for all independent, time-constant factors that influence our dependent variable. (Wooldridge, 2020, pp.439-440)
...
**Task:** Run a regression for Model $(3)$. The implementation in R can be done as usual with the command `lm()`. 

```{r "6_6",eval=FALSE, optional=TRUE}
# Enter your code here.
```


Another more comfortable possibility is to use the function `felm` from the `lfe` package. This allows to add the fixed effects of the regression with the normal control variables via a less computationally intensive and more clearer way. The structure of the felm function is `felm(y ~ x1 + x2 | fixed_effects , data=dat)`. Note that the variables used in the section fixed effects, should not be the dummy variables. It is a single column from the data that is referred to and in which the different categories are listed. 
This means that when using the `lm()` function, the three or two dummy variables are used. `composer1` corresponds to Beethoven, `composer2` corresponds to Liszt and `composer3` corresponds to Mozart. Using the `felm()` function, we refer to the column `composer`, which is a combination of the three previous variables and takes the values Beethoven, Mozart or Liszt.

**Task:** Perform the fixed effects regression for model $(3)$ using the `felm` function.
```{r "6_7"}
...
The relevance condition in this case is $Cor(death\_of\_relative,negemo)\ne0$. We have already briefly mentioned that it is relatively easy to test the validity of this condition. We only have to regress the endogenous variable `negemo` on the instrument and see if the explanatory content of the variation $R^2$ is high enough.
...
Ideally, the instrument should be part of a perfectly randomized experiment, which is unfortunately not the case here. However, if the death of a relative (`death_of_relative`) was random or unexpected, i.e. independent of certain characteristics, this variable is a candidate for an instrument. 
...
Borowiecki does not discuss such reasons in detail in his paper.  Rather, he lists biogafic information on sudden causes of death, to argue for the independence between social status and catching an illness. As a further supporting argument that death is random, he argues that in the results of the first stage regression, with the addition of more variables, the coefficient for `death_of_relative` does not change (Borowiecki 2017, p.602). At this point, it is easy to see that the reasoning used plays a large role in whether an IV estimator can be identified as appropriate. Since Borowiecki substantiates his argumentation with meaningful figures and since we are dealing specifically with the case of three famous composers, the arguments discussed in the previous paragraph can probably not be transferred and we can probably say that the probability of becoming ill was randomly distributed for the persons studied.
...
![Figure 3: Causal graph violating the exogeneity condition ; Source: own diagram.](images/CausalGraphIV2.png)
...
**Task:**  Fill in the gaps to run the IV regression. Do not be surprised by the large number of variables in the IV regression. The function `ivreg()` does not allow for the explicit integration of fixed effects as `felm()` does.
...
Over time, the coefficient on our variable of interest `negemo` became smaller and less significant. That is, until we observed a jump in both the magnitude and precision of the estimate with the introduction of the IV estimator. When we compared the short and long model in the previous chapter we assumed an overestimation of the true coefficient. However, when we compare the OLS results at the beginning with the IV results at the end, we see that we underestimated the true effect. This change in the direction of the bias clearly shows that there are individual effects that point in different directions. In general, any premature interpretation of effects should be treated with caution.

Looking at the Adjusted $R^2$, we see that our added control variables and fixed effects make a substantial contribution to increasing the explanatory power of the models. Note, however, that the interpretation of (Adjusted) $R^2$ in instrumental variables regressions is not useful. This is beause one of the explanatory variables is correlated with the error term and we therefore can't decompose the variance of the outcome y.
...
Quiz: Please choose the most suitable word that fills the gap. *An increase in the average proportion of negative emotions in the total word count by one percentage point, _____ an increase in the annual output of outstanding compositions by 2.5 (2.537) pieces in the following year.*
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Causal Interpretation")
```
...
**Task:**If the average proportion of negative emotions in the total word count increases by 0.1 percentage points (approximately 9.35 percentage), how much (in percentage) does the annual output of outstanding compositions increases in the following year?
...
We see that the instrumental variable shows a positive significant effect for each of the regressions. The effect is strongest for the First Stage regression with `sadness`. Furthermore, only in the instrumental variable regression with `sadness`  a significant effect is observable. For the IV regressions with `anxiety` and `anger` the corresponding effect lies outside the 90% confidence interval (p-value < 0.12). As Borowiecki summarizes, his results are in line with psychological research by Monroe et al. (2009) who finds a strong link between sadness and depression and Andreasen (2005) who finds that depression leads to increased creativity. Also it is consistent with psychological research, that in the First Stage Regression for Sadness, the number of letters written per year has a significant negative effect (i.e. fewer letters are associated with greater sadness) because Goleman (2012) lists isolation and withdrawal as coping mechanisms for sadness.
...
picture<-load.image("images/PictureSol.png")
...
*All of the above links were accessable as of January 07,2022*
